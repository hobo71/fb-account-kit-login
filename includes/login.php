...
add_action( 'login_form', 'fbak_add_html_to_login_form' );
...
function fbak_add_login_scripts() {
    ...
    // both login method enable check
    if ( fbak_enable_both_login_method() ) {
        wp_enqueue_style( 'fbak-login-css' );
...
function fbak_add_html_to_login_form() {
    if ( fbak_enable_both_login_method() ) {
        require_once plugin_dir_path( __FILE__ ) . 'public/views/wp-login.php';
    }
}
