...
function fbak_woocommerce_element_init() {
    ...
    if( fbak_enable_sms_login_method() || fbak_enable_email_login_method() ) {
        ...
        add_action( 'wp_footer', 'fbak_autofill_woocommerce_new_user_password' );
    }
}
...
function fbak_autofill_woocommerce_new_user_password() {
    // display only if already logged in
    ...
    if( function_exists( 'is_account_page' ) && is_account_page() ) {
        ...
        <script>
            jQuery(document).ready(function($) {
                $('input#password_current').val('<?php echo esc_attr( $user->ackit_woo_passcode ); ?>').attr({ type:'text', title:'<?php _e( 'This is the autogenerated password. Please remember it or change this password to relogin to your account using your username and password in the default login form. Note that, if you save changes your profile details, you cannot see this password again.', 'fb-account-kit-login' ); ?>', readonly: 'readonly'});
            });
        </script>
        <?php }
    }
...
function fbak_add_passcheck_to_edit_account_form() {
    $user = wp_get_current_user();
    if( !empty( $user->ackit_woo_passcode ) && $user->ackit_woo_passcheck !== 'yes' ) { ?>
        <input type="hidden" name="ackit_woo_passcheck" id="ackit-woo-passcheck" value="yes" /><div class="clear"></div>
    <?php }
}

function fbak_save_ackit_account_details( $user_id ) {
    // For Favorite color
    if( isset( $_POST['ackit_woo_passcheck'] ) && $_POST['ackit_woo_passcheck'] == 'yes' ) {
