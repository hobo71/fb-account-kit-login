...
function fbak_rating_admin_notice() {
    ...
    $dismiss = wp_nonce_url( add_query_arg( 'fbak_rating_notice_action', 'dismiss_rating_true' ), 'dismiss_rating_true' ); 
    $no_thanks = wp_nonce_url( add_query_arg( 'fbak_rating_notice_action', 'no_thanks_rating_true' ), 'no_thanks_rating_true' ); ?>
    
    <div class="notice notice-success">
        <p><?php _e( 'Hey, I noticed you\'ve been using Facebook Account Kit Login for more than 1 week – that’s awesome! Could you please do me a BIG favor and give it a <strong>5-star</strong> rating on WordPress? Just to help us spread the word and boost my motivation.', 'fb-account-kit-login' ); ?><p>
        <a href="https://wordpress.org/support/plugin/fb-account-kit-login/reviews/?filter=5#new-post" target="_blank" class="button button-secondary"><?php _e( 'Ok, you deserve it', 'fb-account-kit-login' ); ?></a>&nbsp;
        <a href="<?php echo $dismiss; ?>" class="already-did"><strong><?php _e( 'I already did', 'fb-account-kit-login' ); ?></strong></a>&nbsp;<strong>|</strong>
        <a href="<?php echo $no_thanks; ?>" class="later"><strong><?php _e( 'Nope&#44; maybe later', 'fb-account-kit-login' ); ?></strong></a>
    </div>
<?php
}
...
function fbak_dismiss_rating_admin_notice() {
    ...
    if ( 'dismiss_rating_true' === $_GET['fbak_rating_notice_action'] ) {
        check_admin_referer( 'dismiss_rating_true' );
        update_option( 'fbak_plugin_dismiss_rating_notice', '1' );
    }

    if ( 'no_thanks_rating_true' === $_GET['fbak_rating_notice_action'] ) {
